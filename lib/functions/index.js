...
const fs = require("fs");
const os = require("os"); // Correctly included for os.tmpdir()
...
exports.handler = async function (event) {
  ...
  return new Promise((resolve, reject) => {
    ...
    exec(`clamscan ${tmpFilePath}`, (scanError, scanStdout, scanStderr) => {
      if (scanError) {
      ...
      console.log("ClamAV scan output:", scanStdout);

      if (scanStdout.includes("Infected files: 0")) {
        s3.putObject({
        ...
        console.error("File is infected or an error occurred.");
        reject({
          statusCode: 500,
